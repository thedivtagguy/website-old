...
const GetTotallyRealTotalTrips = ( x, distancesOptions = {
	...
	for ( let i = 0; i < x; i++ ) {
		...
		if ( isLong ) {
			...
			lDist = Math.ceil( Math.random() * ( distancesOptions.maxLongDistancesWhenDay -
				distancesOptions.minLongDistancesWhenDay ) ) + distancesOptions.minLongDistancesWhenDay;
			longDistances = longDistances + lDist;
			// Store the randomly chosen short distance.
			sDist = Math.ceil( Math.random() * ( distancesOptions.maxShortDistancesWhenDay -
				distancesOptions.minShortDistancesWhenDay ) ) + distancesOptions.minShortDistancesWhenDay;
			shortDistances = shortDistances + sDist;
			t = lDist * distancesOptions.longDistanceTime + sDist * distancesOptions.longDistanceTime;
		}
		else {
			lDist = Math.ceil( Math.random() * ( distancesOptions.maxLongDistancesWhenNight -
				distancesOptions.minLongDistancesWhenNight ) ) + distancesOptions.minLongDistancesWhenNight;
			longDistances = longDistances + lDist;
			sDist = Math.ceil( Math.random() * ( distancesOptions.maxShortDistancesWhenNight -
				distancesOptions.minShortDistancesWhenNight ) ) + distancesOptions.minShortDistancesWhenNight;
			shortDistances = shortDistances + sDist;
	...
	time = time + longDistances * distancesOptions.longDistanceTime +
		shortDistances * distancesOptions.longDistanceTime;
	time1 = time1 + time;
	console.log( time );
...
function handleResize2() {
	...
	figure2.style( "top", figure2MarginTop + "px" );
	// 3. tell scrollama to update new element dimensions
	scroller2.resize();
}
// scrollama event handlers
...
function handleStepProgress3( response ) {
	...
	counter.innerText = "₹ " + roundedToFixed( response.progress * tripInfo.totalPay,
		1 );
	deliveries.innerText = roundedToFixed( response.progress * ( tripInfo.longTrips +
		tripInfo.shortTrips ), 0 ) + " deliveries";
	timer.innerText = time_convert( roundedToFixed( response.progress * tripInfo.time,
		0 ) );
...
function init() {
	...
	scroller3.setup( {
		...
		progress: true,
		offset: 0.90,
	} ).onStepEnter( handleStepEnter3 ).onStepExit( handleStepExit3 ).onStepProgress(
		handleStepProgress3 );
	// setup resize event
